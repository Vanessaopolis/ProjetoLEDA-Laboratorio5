...
public class MainUsuarioOrdAlfabetica {
	public static void main(String[] args) {
		String main = "MainUsuarioOrdAlfabetica";
		...
		try {
			...
			for (int carga : Main.CARGAS) {
				...
				for (int i = 0; i < Main.REPETICOES; i++) {
					...
					tempoTotalHM += executaListagemOrdemAlfabetica(repoHashMap);
					tempoTotalHS += executaListagemOrdemAlfabetica(repoHashSet);
					tempoTotalLHM += executaListagemOrdemAlfabetica(repoLinkedHashMap);
					tempoTotalTM += executaListagemOrdemAlfabetica(repoTreeMap);
					tempoTotalTS += executaListagemOrdemAlfabetica(repoTreeSet);
				}
			...
			BufferedWriter writer = new BufferedWriter(new FileWriter("/home/ubuntu/ProjetoLEDA-Laboratorio5/data/resultadoUsuarioOrdAlfabetica.txt"));			
